...
func TestCustomerCreateAndFetch(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, customer.CreateInput{
		Name: "Test Farmer", LocalName: "சோதனை விவசாயி",
...
// TestCustomerCreate_CodeIsAutoGeneratedAndSequential replaces a test that
// used to check a caller-supplied duplicate customer_code was rejected —
// that's no longer possible to even attempt, since the caller can't
// supply a code at all anymore (see entitycode.Generate). What actually
// matters now: every create gets a real, non-empty, CUST-prefixed code,
// and two consecutive creates never collide.
func TestCustomerCreate_CodeIsAutoGeneratedAndSequential(t *testing.T) {
	db := connectTest(t)
	defer db.Close()
	tenantID := seedTenant(t, db)
	svc := customer.NewService(db)

	first, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "First"})
	if err != nil {
		t.Fatalf("first create: %v", err)
	}
	second, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Second"})
	if err != nil {
		t.Fatalf("second create: %v", err)
	}
	if first.CustomerCode == "" || second.CustomerCode == "" {
		t.Fatalf("expected non-empty auto-generated codes, got %q and %q", first.CustomerCode, second.CustomerCode)
	}
	if first.CustomerCode == second.CustomerCode {
		t.Fatalf("expected distinct codes, both got %q", first.CustomerCode)
	}
	if !strings.HasPrefix(first.CustomerCode, "CUST-") {
		t.Fatalf("expected a CUST- prefixed code, got %q", first.CustomerCode)
	}
}
...
func TestCustomerList_SearchesByNameCodeAndPhone(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Findable Farmer", Phone: "9998887770"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	if _, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Someone Else", Phone: "1112223330"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	if len(byPhone) != 1 || byPhone[0].Name != "Findable Farmer" {
...
func TestCustomerList_ExcludesWalkingCustomer(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Real Registered Customer"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	err := db.WithAdminTx(context.Background(), func(tx pgx.Tx) error {
	...
	if len(all) != 1 || all[0].Name != "Real Registered Customer" {
...
func TestCustomerList_IncludesOutstandingBalance(t *testing.T) {
	...
	withBalance, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Owes Money"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	if _, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Clean Account"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	for _, c := range all {
		balances[c.Name] = c.Balance
	}
	if !balances["Owes Money"].Equal(decimal.RequireFromString("1500.00")) {
		t.Fatalf("expected Owes Money balance 1500.00, got %s (all: %+v)", balances["Owes Money"], all)
	}
	if !balances["Clean Account"].Equal(decimal.Zero) {
		t.Fatalf("expected Clean Account balance 0, got %s", balances["Clean Account"])
	}
}
...
func TestCustomerSetCreditLimit_RequiresExistingCustomerAndNonNegative(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Credit Test"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
...
func TestCustomerListLedger_ReturnsEntriesNewestFirstAndRejectsUnknownCustomer(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, customer.CreateInput{Name: "Ledger Test"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
...
func TestCustomerCreate_RejectsCustomerTypeNotInDBConstraint(t *testing.T) {
	...
	for _, badType := range []string{"RETAIL", "WHOLESALE", "WALK_IN", "made-up"} {
		_, err := svc.Create(context.Background(), tenantID, customer.CreateInput{
			Name: "Bad Type Customer", CustomerType: badType,
		})
	...
	for _, goodType := range []string{"FARMER", "WHOLESALE_DEALER", "AAVIN_SUBCONTRACTOR", "OTHER"} {
		_, err := svc.Create(context.Background(), tenantID, customer.CreateInput{
			Name: "Good Type Customer", CustomerType: goodType,
		})
		if err != nil {
...
func TestCustomerUpdate_ChangesEditableFieldsNeverCode(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, customer.CreateInput{
		Name: "Original Name", Phone: "9000000000", CustomerType: "FARMER",
	})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	if updated.CustomerCode != created.CustomerCode {
		t.Fatalf("customer_code must never change on update: created as %q, now %q", created.CustomerCode, updated.CustomerCode)
	}
...
func TestCustomerSetActive_TogglesStatusWithoutHardDelete(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, customer.CreateInput{
		Name: "Toggle Test", CustomerType: "FARMER",
	})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
