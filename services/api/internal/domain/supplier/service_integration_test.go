...
func TestSupplierCreateAndFetch(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{
		Name: "Test Feed Mill", GSTIN: "29ABCDE1234F1Z5",
...
func TestSupplierCreate_CodeIsAutoGeneratedAndSequential(t *testing.T) {
	db := connectTest(t)
	defer db.Close()
	tenantID := seedTenant(t, db)
	svc := supplier.NewService(db)

	first, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "First Mill"})
	if err != nil {
		t.Fatalf("first create: %v", err)
	}
	second, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Second Mill"})
	if err != nil {
		t.Fatalf("second create: %v", err)
	}
	if first.SupplierCode == "" || second.SupplierCode == "" {
		t.Fatalf("expected non-empty auto-generated codes, got %q and %q", first.SupplierCode, second.SupplierCode)
	}
	if first.SupplierCode == second.SupplierCode {
		t.Fatalf("expected distinct codes, both got %q", first.SupplierCode)
	}
	if !strings.HasPrefix(first.SupplierCode, "SUPP-") {
		t.Fatalf("expected a SUPP- prefixed code, got %q", first.SupplierCode)
	}
}
...
func TestSupplierCreate_ValidatesRequiredFields(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{}); !errors.Is(err, supplier.ErrValidation) {
		t.Fatalf("expected ErrValidation for missing name, got: %v", err)
	}
	if _, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "X", PaymentTermsDays: -1}); !errors.Is(err, supplier.ErrValidation) {
...
func TestSupplierList_SearchesByNameCodeAndGstin(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Findable Mill", GSTIN: "29AAAAA0000A1Z1"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	if _, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Someone Else Mill"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	if len(byGstin) != 1 || byGstin[0].Name != "Findable Mill" {
...
func TestSupplierList_IncludesOutstandingPayable(t *testing.T) {
	...
	owed, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Owed Mill"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	if _, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Settled Mill"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	for _, s := range all {
		payables[s.Name] = s.Payable
	}
	if !payables["Owed Mill"].Equal(decimal.RequireFromString("8000.00")) {
		t.Fatalf("expected Owed Mill payable 8000.00, got %s (all: %+v)", payables["Owed Mill"], all)
	}
	if !payables["Settled Mill"].Equal(decimal.Zero) {
		t.Fatalf("expected Settled Mill payable 0, got %s", payables["Settled Mill"])
	}
}
...
func TestSupplierListLedger_ReturnsEntriesNewestFirstAndRejectsUnknownSupplier(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Ledger Test Mill"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
...
func TestSupplierUpdate_RevisesFieldsButNeverSupplierCode(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{
		Name: "Original Mill", Phone: "9000000001", PaymentTermsDays: 15,
	})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	...
	if updated.SupplierCode != created.SupplierCode {
		t.Fatalf("supplier_code must never change on update: created as %q, now %q", created.SupplierCode, updated.SupplierCode)
	}
...
func TestSupplierSetActive_NeverHardDeletes(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), tenantID, supplier.CreateInput{Name: "Deactivate Test Mill"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
